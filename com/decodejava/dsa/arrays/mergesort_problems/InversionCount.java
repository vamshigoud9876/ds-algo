...
public class InversionCount {
    ...
    private static int mergeSort(int[] arr, int start, int end, int count) {
        if(start >= end) return 0;
        int mid = start + (end - start) / 2;
        int count1 = mergeSort(arr, start, mid, count);
        int count2 = mergeSort(arr, mid+1, end, count);
        return merge(arr, start, mid, end, count1 + count2);
    }

    String res = "com/decodejava/dsa/arrays/mergesort_problems/resources/inversion_count.png";
